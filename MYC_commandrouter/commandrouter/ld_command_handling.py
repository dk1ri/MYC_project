...
def poll_ld():
    ...
    #***************************************************************
    # unklar war das machn soll
    #**************************************************************
    # v_cr_params.length_commandtoken + 1 (for 0xfd, 0xfe, 0xff) necessary
    if len(v_ld.data_to_cr) >= v_announcelist.length_of_full_elements:
        ...
        if to_def > 0xfc:
            ...
            tokennumber = int.from_bytes(line[0:v_announcelist.length_of_full_elements], byteorder='big', signed=False)
            if tokennumber < v_cr_params.number_of_commands_noCR:
                ...
                if v_ld.ld_len[0] == 0:
                    ...
                    v_ld.ld_len[0] = v_linelength.command[tok][1]
# some switches, no parameters
                if line_length_index_0 == "0":
                    v_ld.ld_len[0] = v_announcelist.length_of_full_elements
                    finish = 1
# switches, range commands and om, am numeric, all paramters numeric and fixed
                elif line_length_index_0 == "1":
                    v_ld.len, finish = data_1(line, got_bytes, v_ld.ld_len,v_linelength.command[tok])

# on string
                elif line_length_index_0 == "2":
                    v_ld.len, finish = data_2(line, got_bytes, v_ld.ld_len, v_linelength.command[tok])

# oa
                elif line_length_index_0 == "3":
                    v_ld.len, finish = data_3(line, got_bytes, v_ld.ld_len, v_linelength.command[tok])

# ob
                elif line_length_index_0 == "4":
                    v_ld.len, finish = data_4(line, got_bytes, v_ld.ld_len, v_linelength.command[tok])

    # error
                else:
                    # wrong data
                    v_ld.ld_len[0] = len(v_ld.data_to_cr)
                    finish = 2
            else:
            ...
            if finish == 0:
                ...
                if finish == 1:
                    ...
                    if to_def == 0xfe:
                        ...
                        ba = int_to_ba(v_token_params.dev_token[tok], v_announcelist.length_of_full_elements)
                        # add params
                        # v_ld_len[0] is calculated using line -> + 1
                        ba.extend(v_ld.data_to_cr[1 + v_announcelist.length_of_full_elements:v_ld.ld_len[0] + 1])
                        ...
                        # to SK
                        v_sk.info_to_all.extend(v_ld.data_to_cr[1:v_ld.ld_len[0] + 1])
                    elif to_def == 0xfd:
                        # to CR
                        v_dev.data_to_device[0].append(v_ld.data_to_cr[1:])
                # else finish == 2 : error
                # reset some values of v_ld after command finished
                v_ld.data_to_cr = v_ld.data_to_cr[v_ld.ld_len[0] + 1:]
