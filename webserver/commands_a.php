...
function create_oa($basic_tok){
    ...
    if (!array_key_exists($basic_tok,$_SESSION["ALL"][$device])) {
        ...
        if (array_key_exists($tok, $_SESSION["des"][$device])){
            ...
            if (count($range) > 2) {
                most_simple_selector_for_simple_des($tok, $range, $actual_data[$tok]);
            }
        }
        echo " ";
        # data
        $position = $actual_data[$tok];
    ...
    if (array_key_exists($basic_tok, $_SESSION["o_to_a"][$device])) {
     #   echo "<marquee>" . $actual_data[$tok] . "</marquee><br>";
...
function create_aa($basic_tok) {
    ...
    if (!array_key_exists($basic_tok,$_SESSION["ALL"][$device])) {
        ...
        $i = 0;
        while (!$stop){
            echo "<marquee>" . $actual_data[$basic_tok."d".$i] . "</marquee><br>";
            $i++;
        }
...
function correct_for_send_oa($basic_tok){
    ...
    if (!array_key_exists($basic_tok,$_SESSION["ALL"][$device])) {
        if ($_SESSION["send_ok"] == 1) {
            ...
            if ($_SESSION["send_ok"] == 1) {
                # update correct only
                $actual_data[$basic_tok . "d" . $pos] = $display_data;
                $actual_data[$basic_tok . "m0"] = $pos;
                # basic_tok
