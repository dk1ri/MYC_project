...
function create_of($basic_tok){
    ...
    $m0 = $basic_tok."m0";
    $d0 = $basic_tok."d0";
    $number_of_elements = explode(",", $_SESSION["des"][$_SESSION["device"]][$basic_tok . "m0"])[0];
    if (array_key_exists($d0, $_SESSION["des_name"][$_SESSION["device"]]) and $_SESSION["des_name"][$_SESSION["device"]][$d0] != "") {
        echo $_SESSION["des_name"][$_SESSION["device"]][$d0];
    }
    else{
        echo " up to " . $number_of_elements . " space separated ";
    }
    $type = explode(",", $_SESSION["original_announce"][$_SESSION["device"]][$basic_tok][1])[0];
    echo " " . find_name_of_type($type) . ": ";
    # $_SESSION["actual_data"][$_SESSION["device"]] may contain spaces: space_to_nbsp()
    ...
    if (array_key_exists($basic_tok, $_SESSION["o_to_a"][$_SESSION["device"]])) {
        echo "<marquee>". str_replace(" ","&nbsp;",$_SESSION["actual_data"][$_SESSION["device"]][$_SESSION["o_to_a"][$_SESSION["device"]][$basic_tok]."d0"]) . "</marquee>";
...
function create_af($basic_tok){
    ...
    foreach ($_SESSION["cor_token"][$_SESSION["device"]][$basic_tok] as $value){
        if (strstr($value, "d")){
            echo str_replace(" ","&nbsp;",$_SESSION["actual_data"][$_SESSION["device"]][$value]). "&nbsp;";
        }
    }
    echo "</marquee><br>";
    stack_memory_selector($basic_tok. "m0");
...
function correct_for_send_of($basic_tok){
            ...
            if (is_numeric($type) or $type == "b") {
                ...
                if ($type == "b" and strlen($splitted[$part_of_string]) > 1) {
                    ...
                    if (is_numeric($type)) {
                        ...
                        $data .= dec_hex(strlen($splitted[$part_of_string]), $_SESSION["property_len"][$_SESSION["device"]][$basic_tok][1]);
                    }
                ...
                if ($_SESSION["send_ok"] == 1) {
                    $data .= translate_dec_to_hex($type, $splitted[$part_of_string], $_SESSION["property_len"][$_SESSION["device"]][$basic_tok][1]);
                }
            }
            $part_of_string++;
        }
    ...
    if ($_SESSION["send_ok"] == 1) {
        ...
        # number of elements
        $send .= translate_dec_to_hex("m", count($splitted), $_SESSION["property_len"][$_SESSION["device"]][$basic_tok][0]);
        $send .= $data;
...
function correct_for_send_af($basic_tok){
    if (array_key_exists($basic_tok . "a", $_POST) and $_POST[$basic_tok . "a"] == 1) {
        if (array_key_exists($basic_tok . "m0", $_POST)) {
            ...
            $value = $_POST[$_SESSION["a_to_o"][$_SESSION["device"]][$basic_tok] . "m0"];
        }
        if (!is_numeric($value)) {
            # manual entry
            # " for numeric#
            # $_SESSION["send_ok"] may be set to 0
        ...
        if ($value > explode(",", $_SESSION["des"][$_SESSION["device"]][$basic_tok . "m0"])[0]) {
            $_SESSION["send_ok"] = 0;
        }
        if ($value == 0) {
            $_SESSION["send_ok"] = 0;
...
function  receive_f($basic_tok, $from_device){
    ...
    $number_length = $_SESSION["property_len"][$_SESSION["device"]][$basic_tok][2];
    # number of elements:
    ...
    while ($i < $number) {
        ...
        list($data_, $delete_bytes) = update_memory_data($basic_tok . "d0", $from_device_,$_SESSION["property_len"][$_SESSION["device"]][$basic_tok][1]);
        $start += $delete_bytes;
        $data .= $data_." ";
        $i++;
    }
    $_SESSION["actual_data"][$_SESSION["device"]][$basic_tok."d0"] = $data;
    $_SESSION["actual_data"][$_SESSION["device"]][$basic_tok."m0"] = $number;
    # add length of $basictok
    return $start + $_SESSION["property_len"][$_SESSION["device"]][$basic_tok][0];
}
?>