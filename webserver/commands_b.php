...
function create_ob($basic_tok){
    ...
    # data
    foreach ($_SESSION["cor_token"][$_SESSION["device"]][$basic_tok] as $token) {
        if (strstr($token, "d")) {
            if (!strstr($token, "dx")) {
                $type = $_SESSION["type_for_memories"][$_SESSION["device"]][$token];
                echo find_name_of_type($type). " ";
                echo "<input type=text name=" . $token . " size =". find_length_of_displayed_vars($type)."  placeholder =" . str_replace(" ","&nbsp;",$_SESSION["actual_data"][$_SESSION["device"]][$token]) . "><br>";
            }
        }
    }
    if (array_key_exists($basic_tok, $_SESSION["o_to_a"][$_SESSION["device"]])) {
        $token = $_SESSION["o_to_a"][$_SESSION["device"]][$basic_tok];
        $range_m0 = explode(",", $_SESSION["des"][$_SESSION["device"]][$token. "m0"]);
        $range_o0 = explode(",", $_SESSION["des"][$_SESSION["device"]][$token. "o0"]);
        $number_of_elements = $range_m0[0];
        array_splice($range_m0, 0, 1);
        array_splice($range_o0, 0, 1);
        if ($number_of_elements > 1) {
            echo "start at: ";
            simple_selector($token."m0", $range_m0, $_SESSION["actual_data"][$_SESSION["device"]][$token."m0"]);
            echo "number of elements: ";
            simple_selector($token."o0", $range_o0, $_SESSION["actual_data"][$_SESSION["device"]][$token."o0"]);
        }
...
function create_ab($basic_tok) {
    ...
    if ($number_of_elements > 1) {
        echo "start at ";
        simple_selector($tok, $range, $_SESSION["actual_data"][$_SESSION["device"]][$tok]);
    }
    else {
        # one type of data
        echo find_name_of_type($_SESSION["type_for_memories"][$_SESSION["device"]][$basic_tok. "d0"]);
    }
    # number of elements
    $tok = $basic_tok. "o0";
    $range = explode(",", $_SESSION["des"][$_SESSION["device"]][$tok]);
    ...
    # data
    if ($number_of_elements == 1){
        $data = str_replace(" ", "&nbsp;", $_SESSION["actual_data"][$_SESSION["device"]][$basic_tok . "d0"]);
    }
    else {
        $data = "";
        $i = 0;
        $j = $_SESSION["actual_data"][$_SESSION["device"]][$basic_tok . "m0"];
        while ($i < $_SESSION["actual_data"][$_SESSION["device"]][$tok]) {
            if ($j >= explode(",", $_SESSION["des"][$_SESSION["device"]][$basic_tok . "m0"])[0]) {
                $j = 0;
            }
            $data .= str_replace(" ", "&nbsp;", $_SESSION["actual_data"][$_SESSION["device"]][$basic_tok . "d" . $j]) . "&nbsp;.&nbsp;";
            $i++;
            $j++;
        }
    }
...
function correct_for_send_ob($basic_tok){
    # $_SESSION["actual_data"][$_SESSION["device"]] data is not updated
    ...
    if ($_SESSION["send_ok"]) {
        ...
        $start_pos = 0;
        foreach ($_SESSION["cor_token"][$_SESSION["device"]][$basic_tok] as $value) {
                ...
                if (strstr($value, "d")) {
                    if (!strstr($value, "dx")) {
                        ...
                        if (array_key_exists($value, $_POST) and $_POST[$value] != "") {
                            ...
                            $type = $_SESSION["type_for_memories"][$_SESSION["device"]][$value];
            ...
            if ($stop == 0) {
                if (strstr($value, "d")) {
                    if (!strstr($value, "dx")) {
                        ...
                        if (array_key_exists($value, $_POST) and $_POST[$value] != "") {
                            $_SESSION["actual_data"][$_SESSION["device"]][$value] = $_POST[$value];
                            $start = 1;
                        } else {
                            if ($start == 1) {
                                $stop = 1;
...
function correct_for_send_ab($basic_tok){
    ...
    if (array_key_exists($basic_tok . "a", $_POST) and $_POST[$basic_tok . "a"] == "1") {
        $m0 = $basic_tok . "m0";
        $o0 = $basic_tok . "o0";
        if (count(explode(",", $_SESSION["des"][$_SESSION["device"]][$basic_tok . "m0"])) > 3) {
            # more than 1 element
            $number = $_POST[$o0];
            if ($number > 0) {
            ...
            $_SESSION["actual_data"][$_SESSION["device"]][$m0] = $_POST[$m0];
            $_SESSION["actual_data"][$_SESSION["device"]][$o0] = $_POST[$o0];
        }
        if ($_POST[$o0] > 0) {
            # basic_tok
            $send = translate_dec_to_hex("m", $basic_tok, $_SESSION["property_len"][$_SESSION["device"]][$basic_tok][0]);
...
function receive_b($basic_tok, $from_device){
    ...
    $max_element_number = count($_SESSION["original_announce"][$_SESSION["device"]][$basic_tok]) - 2;
