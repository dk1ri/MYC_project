"""
name : init.py
last edited: 20190312
"""

import os

from io_handling import *
from misc_functions import *

import v_sk
import v_Ic7300_vars
import v_announcelist

def initialization():
    # read config
    readconfig(sys.argv)
    create_announce_list()
    # read info for all devices and initialize them
    # and create announcelist
 #   new_device_found = read_my_devices()
    initialize_inputbuffer("TERMINAL")
    initialize_inputbuffer("TELNET")
    return

def readconfig(arg):
    if len(arg) > 1:
        v_Ic7300_vars.config_file = arg[1]
    if not os.path.isfile(v_Ic7300_vars.config_file):
        create_config_file(v_Ic7300_vars.config_file, "a")
    config_file = open(v_Ic7300_vars.config_file)
    i = 0
    # sequence within file is fixed!!!
    for lines in config_file:
        if lines[0] == "#":
            continue
        lines = lines.rstrip()
        if i == 0:
            v_Ic7300_vars.announce_list = lines
            if not os.path.isfile(v_Ic7300_vars.announce_list):
                sys.exit("Missing:  " + v_configparameter.annoucments)
        if i == 1:
            # time in sec for command timeout
            v_Ic7300_vars.command_timeout = int(lines)
        if i == 2:
            v_Ic7300_vars.log_file = lines.rstrip()
            # create is not exist
            open(v_Ic7300_vars.log_file, "a").close()
        if i == 3:
            # logfile size
            v_Ic7300_vars.logfilesize = int(lines)
        if i == 4:
            # test mode
            v_Ic7300_vars.test_mode = int(lines)
            v_Ic7300_vars.test_mode = int(lines)
        if i == 5:
            v_Ic7300_vars.name = lines.rstrip()
        if i == 6:
            v_Ic7300_vars.number = int(lines)
        if i == 7:
            v_Ic7300_vars.terminal_activ = int(lines)
        if i == 8:
            v_Ic7300_vars.telnet_active = int(lines)
        if i == 9:
            v_Ic7300_vars.telnet_port = int(lines)
        if i == 10:
            v_Ic7300_vars.comport = int(lines)
            v_Ic7300_vars.comport = "COM" + str(v_Ic7300_vars.comportnumber)
        if i == 11:
            v_Ic7300_vars.civ_address = int(lines)
        if i == 12:
            v_Ic7300_vars.civ_watchdog_timeout = int(lines)
        i += 1
    config_file.close()
    return

def create_config_file (file, write_append):
    handle = open(file, write_append)
    handle.write("# sequence must not be changed !!!\n")
    handle.write("# line 0 filename for announcements\n")
    handle.write(str(v_Ic7300_vars.announce_list) + "\n")
    handle.write("# line 1: time in sec for command timeout\n")
    handle.write(str(v_Ic7300_vars.command_timeout) + "\n")
    handle.write("# line 2 filename for logfile\n")
    handle.write(str(v_Ic7300_vars.log_file) + "\n")
    handle.write("# line 3: size of logfile; all lines are purged then\n")
    handle.write(str(v_Ic7300_vars.logfilesize) + "\n")
    handle.write("# line 4: testmode\n")
    handle.write(str(v_Ic7300_vars.test_mode) + "\n")
    handle.write("# line 5: Name\n")
    handle.write(v_Ic7300_vars.device_name + "\n")
    handle.write("# line 6: Number\n")
    handle.write(v_Ic7300_vars.device_number + "\n")
    handle.write("# line 7: Terminal active\n1\n")
    handle.write(v_Ic7300_vars.terminal_activ + "\n")
    handle.write(" line 8:Telnet active\n")
    handle.write(v_Ic7300_vars.telnet_active  + "\n")
    handle.write(" # line 9: Telnet Port\n")
    handle.write(v_Ic7300_vars.telnet_port + "\n")
    handle.write(" # line10: Com Port\n")
    handle.write(v_Ic7300_vars.comportnumber + "\n")
    handle.write("# line 11: CIV Adress (0x94 = 148)\n")
    handle.write(v_Ic7300_vars.civ_address + "\n")
    handle.write("# line12: CIV timeout (seconds)\n")
    handle.write(v_Ic7300_vars.civ_watchdog_timeout + "\n")
    handle.close()
    return


def initialize_inputbuffer(line):
    # initialize inputs for SK interface (Terminal and telnet
    m = create_inputbuffer(line, 1)
    if line == "TELNET":
        v_sk.socket[m] = 23
        start_ethernet_server(v_sk.socket[m], m)

def create_inputbuffer(interface_type, activ,):
    v_sk.interface_port.append(0)
    v_sk.interface_timeout.append(0)
    v_sk.interface_type.append(interface_type)
    v_sk.socket.append([])
    v_sk.ethernet_server_started.append([])
    create_sk()
    return len(v_sk.ethernet_server_started) - 1

def create_sk():
    v_sk.active.append(1)
    v_sk.inputline.append([])
    # actual_call in analyze_length for next action:
    v_sk.len.append([0, 0, 0, 0, 0])
   # v_sk.last_error.append("no error")
    v_sk.starttime.append(0)
    return

def create_announce_list():
    config_file = open("___announcements")
    i = 0
    v_announcelist.full = []
    for lines in config_file:
        if lines[0] != "D":
            continue
        lines = lines.rstrip()
        lines_ = lines.split("\"")
        lin =  add_length_to_str(lines_[1], 1)
        v_announcelist.full.append(lin)
        i += 1
    v_announcelist.number_of_lines = i