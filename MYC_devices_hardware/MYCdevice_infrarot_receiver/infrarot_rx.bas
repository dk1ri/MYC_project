...
Config Rc5 = Pinb.0 , Timer = 1 , Mode = background, Wait = 100
Enable Interrupts
'
DDRB.2 = 0
'Input
...
If _rc5_bits.4 = 1 Then
   ...
   Tempb = Rc5_address And &B00011111
   Tempd = Rc5_command And &B00111111
   If No_myc = 1 Then
      'Rc5_adress
      Tempc = Tempb / 10
      Tempc = Tempc + 48
      ...
      Printbin Tempc
   End If
   If Tempb = Valid_adress Then
      Rc5buffer_b(Rc5_writepointer) = Tempd
...
'Data "0;m;DK1RI;Infrared (RC5) receiver;V04.0;1;145;1;11;1-1"
      I2c_tx_busy = 2
      Tx_time = 1
      A_line = 0
      Number_of_lines = 1
...
'Data "1;aa,RC5 buffer;252,{&H00 to &H3F}"
      ...
      If Rc5_overflow = 0 Then
         I2c_tx_b(2) = Rc5_writepointer - 1
         'length
         If Rc5_writepointer > 1 Then
            For Tempb = 1 To Rc5_writepointer - 1
               Tempc = Tempb + 2
               'Rc5buffer start with 1
               I2c_tx_b(Tempc) = Rc5buffer_b(Tempb)
            Next Tempb
         End If
         I2c_write_pointer = Rc5_writepointer + 2
      Else
         I2c_tx_b(2) = Rc5_writepointer
         'all bytes  of Rc5_buffer are transmitted
         Tempc = Rc5_writepointer
         For Tempb = 1 to Rc5_stringlength
            Tempd = Tempb + 2
            I2c_tx_b(Tempd) = Rc5buffer_b(Tempc)
            Incr Tempc
            If Tempc > Rc5_stringlength Then Tempc = 1
         Next Tempb
         I2c_write_pointer = Rc5_stringlength + 2
      End if
      Gosub Reset_rc5buffer
      If Command_mode = 1 Then Gosub Print_i2c_tx
      Gosub Command_received
