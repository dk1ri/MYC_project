...
'
Set_sleep_mode0:
   SPI_IO_b(1) = RegOpMode_w0
   SPI_IO_b(2) = &H80
   Call Write_spi(2)
   '
   SPI_IO_b(1) = RegOpMode_w0
   SPI_IO_b(2) = RegOpMode_standby0
   Call Write_spi(2)
Return
'
Read_irq0:
   SPI_IO_b(1) = RegIrqFlags0
   Call Read_spi(1)
Return
'
Reset_irq0:
   SPI_IO_b(1) = RegIrqFlags_w0
   SPI_IO_b(2) = &HFF
   Call Write_spi(2)
   Waitms 1
   Call Write_spi(2)
...
Set_frequency_0:
   ...
   D_temp1 = Si_temp_w1
   SPI_IO_b(1) = RegFrMsb_w0
   SPI_IO_b(2) = D_temp1_b(3)
   SPI_IO_b(3) = D_temp1_b(2)
   SPI_IO_b(4) = D_temp1_b(1)
   Call write_spi(4)
Return
'
Set_rx_start0:
...
RFM95_send0:
   ...
   SPI_IO_b(2) = wireless_tx_length - 1
   Call Write_spi (2)
   '
   waitms 1
   '
   SPI_IO_b(1) = RegOpMode_w0
   SPI_IO_b(2) = RegOpMode_tx0
   ' send
   Call Write_spi(2)
   Stop WAtchdog
   B_temp2 = 0
   Do
      Gosub Read_irq0
      B_temp1 = SPI_IO_b(1) And &B00001000  ' tx done
...
RFM95_receive0:
   ...
   If B_temp1 > 0 Then
      ...
      ElseIf B_temp3 = Irq_rx_done Then
         ' Rx done
         SPI_IO_b(1) = RegFifoAddrPtr_w0
         SPI_IO_b(2) = Reg_fifo_start_rx0
         Call Write_spi(2)
         '
         SPI_IO_b(1) = FifoRxNBBytes0
         Call Read_spi(1)
         B_temp1 = SPI_IO_b(1)
         '
         SPI_IO_b(1) = Reg_fifo0
...
'
         ...
         For B_temp4 = 1 To B_temp3
            If Radio_name_b(B_temp4) <> SPI_IO_b(B_temp4) Then B_temp2 = 0
         Next B_temp4
         If B_temp2 = 1 Then
            'name ok
            Incr B_temp3
   ...
   '            Tx_write_pointer = 1
               For B_temp2 = B_temp3 To B_temp1
                  command_b(B_temp4) = SPI_IO_b(B_temp2)
                  Incr Commandpointer
                  Incr B_temp4
               Next B_temp2
            End If
